...
```sql
SELECT pg_replication_origin_create('pgmanager-a')
WHERE NOT EXISTS (
    SELECT 1 FROM pg_replication_origin WHERE roname = 'pgmanager-a'
)
```
...
```sql
SELECT pg_replication_origin_session_setup('pgmanager-a')
```
...
```sql
SELECT pg_drop_replication_slot('pgmanager')
```
...
| `roname` | `text` | The origin name (e.g., `"pgmanager-a"`) |
...
`pg_replication_origin_session_setup()` sets the origin for the entire session (connection lifetime), not per-transaction. This is more efficient than per-transaction origin setup and matches pgmanager's connection model where the applier uses a dedicated pool.

## Thread Safety

The `Conn` wrapper is **not** thread-safe. It wraps a single `pgconn.PgConn` which is a stateful protocol connection. All calls to a `Conn` instance must be serialized by the caller. In pgmanager, this is naturally enforced because:
