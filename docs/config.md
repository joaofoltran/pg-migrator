...
This is the single configuration object passed to `pipeline.New()` and shared across all components. It is populated by cobra flag binding in `cmd/pgmigrator/root.go`.
...
type ReplicationConfig struct {
    SlotName     string   // Replication slot name (default: "pgmigrator")
    Publication  string   // Publication name (default: "pgmigrator_pub")
...
|-------|----------|---------|-------------|
| `SlotName` | `--slot` | `pgmigrator` | Name of the logical replication slot on the source |
| `Publication` | `--publication` | `pgmigrator_pub` | PostgreSQL publication that defines which tables to replicate |
...
The `pgoutput` plugin is PostgreSQL's built-in logical decoding output plugin, available since PostgreSQL 10. It produces binary-encoded messages (RelationMessage, BeginMessage, InsertMessage, etc.) that map directly to pgmigrator's `stream.Message` types. It's preferred over alternatives like `wal2json` because:
...
The config struct is populated via cobra's persistent flags in `cmd/pgmigrator/root.go`:
...
func init() {
    ...
    // Replication
    f.StringVar(&cfg.Replication.SlotName, "slot", "pgmigrator", "...")
    f.StringVar(&cfg.Replication.Publication, "publication", "pgmigrator_pub", "...")
