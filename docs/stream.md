...
The stream package is the architectural spine of pgmanager. It defines the unified `Message` interface through which all data flows — both real WAL changes and synthetic sentinels — and the `Decoder` that consumes the PostgreSQL logical replication stream and converts it into these messages.
...
| `slotName`  | Replication slot name (e.g., `pgmanager`)          |
| `publication`| Publication name (e.g., `pgmanager_pub`)          |
